...
func RegisterUserRoutes(mux *http.ServeMux, pool *pgxpool.Pool, redisClient *redis.Client, resendClient *resend.Client, trustedProxyNet *net.IPNet) {
	...
	mux.HandleFunc("/createUser", middleware.CheckUserIPandVerify(httpcallers.CallCreateUser(pool), redisClient, trustedProxyNet, redisInternal.FormTokenCost))
	mux.HandleFunc("/newUser", middleware.CheckUserIPandVerify(httpcallers.CallNewUser(), redisClient, trustedProxyNet, redisInternal.PageTokenCost))
	mux.HandleFunc("/loginUser", middleware.CheckUserIPandVerify(httpcallers.CallLoginUser(), redisClient, trustedProxyNet, redisInternal.PageTokenCost))
	mux.HandleFunc("/verifyLogin", middleware.CheckUserIPandVerify(httpcallers.CallVerifyUser(pool), redisClient, trustedProxyNet, redisInternal.FormTokenCost))
	...
	mux.HandleFunc("/sendVerificationMail", middleware.CheckUserIPandVerify(httpcallers.CallSendVerificationMail(resendClient, pool, redisClient), redisClient, trustedProxyNet, redisInternal.FormTokenCost))
	mux.HandleFunc("/inputOTP", middleware.CheckUserIPandVerify(httpcallers.CallVerifyOTPclientSide(), redisClient, trustedProxyNet, redisInternal.PageTokenCost))
	mux.HandleFunc("/verifyOTP", middleware.CheckUserIPandVerify(httpcallers.CallVerifyOTPserverSide(redisClient, pool), redisClient, trustedProxyNet, redisInternal.FormTokenCost))
